...
import { SuburbSummary, ProcessedData, ProcessingMetadata } from '../types/index';
import { OUTPUT_CONFIG } from '../config/constants';
import { logger } from '../utils/logger';

export class DataPublisher {
  async publish(summaries: SuburbSummary[], outputDir: string): Promise<void> {
  ...
  private createLookupData(summaries: SuburbSummary[]): Record<string, any> {
  ...
  private generateStatistics(summaries: SuburbSummary[]): any {
    const stats = {
      generated: new Date().toISOString(),
      summary: {
      ...
      collectionDays: {} as Record<string, number>,
      zones: {} as Record<string, number>,
      warnings: [] as string[]
    };

    // Calculate averages and counts
    stats.summary.averagePropertiesPerSuburb = Math.round(
      stats.summary.totalProperties / stats.summary.totalSuburbs
    );
