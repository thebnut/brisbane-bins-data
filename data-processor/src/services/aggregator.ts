...
import { DATA_QUALITY_THRESHOLDS } from '../config/constants';
import { logger } from '../utils/logger';

export class SuburbAggregator {
  aggregate(properties: PropertyCollection[]): SuburbSummary[] {
    const suburbMap = new Map<string, PropertyCollection[]>();
  ...
  private processSuburb(suburb: string, properties: PropertyCollection[]): SuburbSummary {
  ...
  private calculateStats(properties: PropertyCollection[]): SuburbStats {
    const total = properties.length;
  ...
  private calculateCollectionInfo(properties: PropertyCollection[]): CollectionInfo {
    ...
    const total = properties.length;
    const schedule: DayStats[] = Array.from(dayCounts.entries())
      .map(([day, count]) => ({
        day,
        count,
  ...
  private calculateZoneInfo(properties: PropertyCollection[]): ZoneInfo {
    ...
    const total = properties.length;
    const distribution: ZoneStats[] = Array.from(zoneCounts.entries())
      .map(([zone, count]) => ({
        zone,
        count,
        percentage: Math.round((count / total) * 1000) / 10 // Round to 1 decimal
  ...
  private calculateCompleteness(properties: PropertyCollection[]): number {
    const requiredFields: (keyof PropertyCollection)[] = [
      'property_id', 
      'suburb', 
      'day_of_week', 
      'zone'
  ...
  private normalizeSuburbName(suburb: string): string {
